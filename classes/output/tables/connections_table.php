...
class connections_table extends table_sql {
    ...
    public function col_name($values) {
        ...
        $editurl = new moodle_url('/local/lb_filetransfer/connection_form_page.php', $urlparams);
    ...
    public function col_actions($values) {
        ...
        $urlparams = array('id' => $values->id, 'sesskey' => sesskey());
        $editurl = new moodle_url('/local/lb_filetransfer/connection_form_page.php', $urlparams);
        $deleteurl = new moodle_url('/local/lb_filetransfer/connections.php', $urlparams + array('action' => 'delete'));
        // Decide twixt activate or deactivate.
        if ($values->active) {
            $toggleurl = new moodle_url('/local/lb_filetransfer/connections.php', $urlparams + array('action' => 'hide'));
            $togglename = get_string('deactivate', 'local_lb_filetransfer');
            $toggleicon = 'fa fa-eye';
        } else {
            $toggleurl = new moodle_url('/local/lb_filetransfer/connections.php', $urlparams + array('action' => 'show'));
